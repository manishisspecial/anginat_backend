...
class AuthController {
    async register(req, res) {
        ...
        try {
            ...
            if (['instructor', 'admin', 'super-admin'].includes(role) && !institutionData) {
                await session.abortTransaction();
                return sendErrorResponse(res, "Institution data is required for this role", 400);
            }
            ...
            if (existingInstitutionByDomain) {
                await session.abortTransaction();
                return sendErrorResponse(res, "Institution with this domain already exists", 400);
            }
            ...
            const institution = await InstitutionService.findOrCreateInstitution(institutionData, { session });
            if (!institution) {
                await session.abortTransaction();
    ...
    async createUser(req, res) {
        ...
        session.startTransaction();
        try {
            const { email, phoneNumber, username, password, role, status, name, institutionId } = req.body;
            ...
            if (['instructor', 'admin', 'super-admin'].includes(role) && !institutionId) {
                await session.abortTransaction();
                return sendErrorResponse(res, "Institution ID is required for this role", 400);
            }
    ...
    async refreshToken(req, res) {
        ...
        try {
            ...
            const accessToken = jwt.sign(
                { id: user._id, role: user.role, institution: user.institutionId },
                process.env.ACCESS_TOKEN_SECRET,
                { expiresIn: "1h" }
            );
