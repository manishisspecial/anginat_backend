...
class TimetableService {
  async createTimetable(timetableData) {
    ...
    if (
      ...
      throw new Error(
        "Institution, academicClass, section, subject, instructor, semester, and weeklyHours are required"
      );
    }
    if (
      !mongoose.Types.ObjectId.isValid(institution) ||
    ...
    // Validate subject
    const subjectDoc = await SubjectRepository.getSubjectById(
      subject,
      institution
    );
    if (!subjectDoc) {
      throw new Error(
        "Subject not found or does not belong to your institution"
      );
    }
    // Validate instructor
    ...
    if (
      ...
      throw new Error(
        "Instructor must be an active user with role 'instructor'"
      );
    }
    // Validate instructor is in subject's instructors
    if (!subjectDoc.instructors.some((id) => id.equals(instructor))) {
      throw new Error(
        "Instructor must be one of the subject’s assigned instructors"
      );
    }
    // Validate academicClass
    const classDoc = await AcademicClassRepository.getAcademicClassById(
      academicClass,
      institution
    );
    if (!classDoc) {
    ...
    const sectionDoc = await SectionRepository.getSectionById(
      section,
      institution
    );
    if (!sectionDoc) {
      throw new Error(
        "Section not found or does not belong to the specified academicClass"
      );
    }
    // Validate degree if provided
    if (degree) {
      const degreeDoc = await DegreeRepository.getDegreeById(
        degree,
        institution
      );
      if (!degreeDoc) {
      ...
      if (!classDoc.degree || !classDoc.degree.equals(degree)) {
        throw new Error(
          "AcademicClass does not belong to the specified degree"
        );
      }
    }
    ...
    if (scheduleDetails && scheduleDetails.length > 0) {
      for (const slot of scheduleDetails) {
        if (!slot.day || !slot.startTime || !slot.endTime) {
          throw new Error(
            "Each schedule detail must include day, startTime, and endTime"
          );
        }
      }
    }
  ...
  async getTimetableById(timetableId, institutionId) {
    ...
    const timetable = await TimetableRepository.getTimetableById(
      timetableId,
      institutionId
    );
    if (!timetable) {
      throw new Error(
        "Timetable not found or does not belong to your institution"
      );
    }
    return timetable;
  }
  ...
  async updateTimetable(timetableId, timetableData, institutionId) {
    ...
    if (academicClass) {
      ...
      if (section) {
        const sectionDoc = await SectionRepository.getSectionByIdByClass(
          section,
          academicClass,
          institutionId
        );
    ...
    const updatedTimetable = await TimetableRepository.updateTimetable(
      timetableId,
      timetableData,
      institutionId
    );
  ...
  async deleteTimetable(timetableId, institutionId) {
    ...
    const deletedTimetable = await TimetableRepository.deleteTimetable(
      timetableId,
      institutionId
    );
    if (!deletedTimetable) {
